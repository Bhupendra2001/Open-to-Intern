...
const createCollege = async (req, res) => {
    ...
    try {
        ...
        if (checkname) return res.status(400).send({ status: false, msg: "Pls provide a unique name" })
...
const getCollegeData = async function (req, res) {
    try {
        ...
        let interAvailable = await internModel.find({ collegeId }).select({ _id: 1, name: 1, email: 1, mobile: 1 })
        if (interAvailable.length == 0) { return res.status(404).send({ status: false, msg: `No intern found with this ${collegeName}` }) }
